...
function obterComissao(valor) {
  if (valor < 100) return 0;
  if (valor < 500) return valor * 0.01;
  return valor * 0.05;
}

for (const venda of lista.vendas) {
  const valorComissao = obterComissao(venda.valor);

  if (valorComissao > 0) {
    console.log(
      `Comissão de R$${valorComissao.toFixed(2)} para ${venda.vendedor}`
    );
  }

  const vendedor = comissoes.find((c) => c.nome === venda.vendedor);
  vendedor.valor += valorComissao;
}

console.log("\nRESULTADO FINAL:");
for (const resultado of comissoes) {
  console.log(`${resultado.nome}: R$${resultado.valor.toFixed(2)}`);
}
